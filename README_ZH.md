...
**语境感知文档翻译 / Context-aware document translation**
...
- 下载最新的 `.dmg`
- 打开后把 `ContextWeave.app` 拖到 `Applications`
- 从 `Applications` 启动 `ContextWeave.app`
- 如果 macOS 因为开发者无法验证而阻止启动，打开 `系统设置` -> `隐私与安全性`
- 在 `安全性` 区域里为 `ContextWeave.app` 点击 `仍要打开`，然后再确认 `打开`
...
语络译也包含一个小型 CLI，可用于配置驱动的一次性翻译和基础书库管理。从源码目录运行时使用 `uv run contextweave-cli`；安装成包后使用 `contextweave-cli`。
