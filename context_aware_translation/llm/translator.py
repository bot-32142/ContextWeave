...
判断内容背景与文本风格。
结合术语表描述，分析人物特征与性格。
若输入包含“近期前文摘要”，它是当前文本之前几个块的极短事实摘要，只用于解决省略主语/宾语、指代、说话对象、回应方向或即时场景连续性。
当前原文始终优先于近期前文摘要；若摘要与当前原文无关或冲突，忽略摘要。只翻译“原文”，不得翻译摘要本身，不得把摘要内容扩写为原文没有的信息。
逐元素翻译：
...
async def translate_chunk(
    ...
    with llm_session_scope() as session_id:
        prepared = prepare_chunk_translation(
            chunks, terms, source_language, target_language, local_context=local_context
        )
